...
class DeployManager:
    ...
    def _run_git_clone(self, repo_url: str, version_tag: str, deploy_path: Path) -> bool:
        """
        克隆指定版本到指定路径。
        ...
        try:
            ...
            if process.returncode == 0:
                logger.info(f"成功从 {repo_url} 克隆版本 {version_tag} 到 {deploy_path}")
                ...
                logger.error(f"从 {repo_url} 克隆失败 (版本: {version_tag})。返回码: {process.returncode}")
