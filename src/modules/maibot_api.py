...
async def get_emoji_by_hash(instance_id: str, request: EmojiHashRequest):
    ...
    """
    logger.info(f"收到根据哈希获取表情包请求，实例ID: {instance_id}, 哈希: {request.emoji_hash}")
    
    try:
        result = maibot_resource_manager.get_emoji_by_hash(
            instance_id=instance_id,
            emoji_hash=request.emoji_hash
        )
...
async def search_emojis(instance_id: str, request: EmojiSearchRequest):
    ...
    try:
        # 过滤掉 None 值的参数
        filters = {k: v for k, v in request.dict().items() 
...
@router.post("/resource/{instance_id}/emoji/{emoji_id}/usage", response_model=BaseResponse)
async def increment_emoji_usage(instance_id: str, emoji_id: int):
    ...
    """
    logger.info(f"收到增加表情包使用次数请求，实例ID: {instance_id}, 表情包ID: {emoji_id}")
...
@router.post("/resource/{instance_id}/emoji/{emoji_id}/query", response_model=BaseResponse)
async def increment_emoji_query(instance_id: str, emoji_id: int):
    ...
    - **描述**: 增加表情包查询次数
    """
    logger.info(f"收到增加表情包查询次数请求，实例ID: {instance_id}, 表情包ID: {emoji_id}")
...
@router.get("/resource/{instance_id}/person/record/{record_id}", response_model=DataResponse)
...
async def get_person_info_by_platform_user(instance_id: str, request: PlatformUserRequest):
    """
    根据平台和用户ID获取用户信息
...
async def update_person_info(instance_id: str, person_id: str, request: PersonInfoUpdateRequest):
    """
    更新用户信息
...
@router.delete("/resource/{instance_id}/person/{person_id}", response_model=BaseResponse)
...
@router.post("/resource/{instance_id}/person/{person_id}/interaction", response_model=BaseResponse)
async def update_person_interaction(instance_id: str, person_id: str, request: PersonInfoInteractionRequest):
    """
    更新用户交互信息
...
async def get_all_instances_resources():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"获取所有实例资源信息失败: {str(e)}")
...
async def get_emoji_batch(instance_id: str, request: EmojiBatchRequest):
    ...
    """
    logger.info(f"收到批量获取表情包请求，实例ID: {instance_id}, 批次大小: {request.batch_size}, 偏移: {request.offset}")
    ...
    try:
        ...
        if result["status"] == "success":
            logger.info(f"批量获取表情包成功: 返回 {result.get('returned_count', 0)} 条记录")
            return ListResponse(
...
async def get_person_info_batch(instance_id: str, request: PersonInfoBatchRequest):
    ...
    """
    logger.info(f"收到批量获取用户信息请求，实例ID: {instance_id}, 批次大小: {request.batch_size}, 偏移: {request.offset}")
    ...
    try:
        ...
        if result["status"] == "success":
            logger.info(f"批量获取用户信息成功: 返回 {result.get('returned_count', 0)} 条记录")
            return ListResponse(
                status=result["status"],
                message=result["message"],
                data=result["data"],
