...
async def deploy_maibot(payload: DeployRequest = Body(...)):
    ...
    with Session(engine) as session:
        ...
        deploy_success = deploy_manager.deploy_version(payload.version, payload.install_path)

        if not deploy_success:
            logger.error(f"使用 deploy_manager 部署版本 {payload.version} 到 {payload.install_path} 失败。")
