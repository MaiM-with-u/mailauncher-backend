...
async def get_pty_command_and_cwd_from_instance(instance_id_full: str) -> Tuple[Optional[str], Optional[str], Optional[str]]:
    db = get_db_instance()
    ...
    if type_part == "main":
        ...
        if service_details and service_details.path:
            ...
            logger.info(f"为服务 '{type_part}' 配置 PTY: 命令='{pty_command}', CWD='{pty_cwd}'")
        else:
            logger.warning(f"无法从数据库获取服务 '{type_part}' (实例: '{instance_short_id}') 的详细信息或路径。")
            return None, None, status_value
    else:
        logger.warning(f"未知的 PTY 类型: '{type_part}' (实例: '{instance_short_id}')。无法确定命令。")
        return None, None, status_value

    logger.info(f"实例 {instance_id_full} 的 PTY 配置: 命令='{pty_command}', CWD='{pty_cwd}', 状态='{status_value}'")
    return pty_command, pty_cwd, status_value

async def pty_output_to_websocket_and_db(session_id: str, pty_process: PtyProcess, websocket: WebSocket, db: Database):
    """
        ...
        while True:
            try:
                ...
                try:
                    ...
                    logger.warning(f"会话 {session_id} 发生 Unicode 解码错误，已使用替换字符。")

                if websocket.client_state == WebSocketState.CONNECTED:
                    try:
                        await websocket.send_json({"type": "output", "data": str_data})
                    except RuntimeError as e_send_runtime:
                        logger.warning(f"发送 PTY 输出到 WebSocket (会话 {session_id}) 时发生运行时错误 (可能已关闭): {e_send_runtime}")
                        break
                    except Exception as e_send:
                        logger.error(f"发送 PTY 输出到 WebSocket (会话 {session_id}) 时出错: {e_send}")
                        break
                else:
                    logger.info(f"WebSocket (会话 {session_id}) 不再连接。停止 PTY 输出。")
                    break
                
                await db.add_pty_log(session_id, str_data, MAX_LOG_HISTORY)

            except Exception as e_read:
                logger.error(f"会话 {session_id} 的 PTY 读取错误 (PTY 进程可能已关闭): {e_read}")
                break
    except Exception as e:
        logger.error(f"pty_output_to_websocket_and_db 中发生意外错误 (会话 {session_id}): {e}")
    finally:
        logger.info(f"会话 {session_id} 的 PTY 输出转发任务已完成。")


async def handle_websocket_connection(websocket: WebSocket, session_id: str, db: Database):
    """
    处理单个 WebSocket 连接 (FastAPI version)。
    session_id 是从路径中提取的部分，例如 "yourinstanceid_main"。
    """
    await websocket.accept()
    logger.info(f"客户端已连接 (FastAPI)，会话 ID: {session_id}，来自 {websocket.client.host}:{websocket.client.port}")
    ...
    async with active_ptys_lock:
        if session_id in active_ptys and active_ptys[session_id].get("pty") and active_ptys[session_id]["pty"].isalive():
            ...
            if websocket.client_state == WebSocketState.CONNECTED:
                try:
                    await websocket.send_json({"type": "error", "message": "会话已被其他客户端激活。"})
                except RuntimeError:
                    pass 
            if websocket.client_state == WebSocketState.CONNECTED:
                await websocket.close(code=1008)
    ...
    try:
        ...
        if historical_logs:
            logger.info(f"正在向客户端发送 {len(historical_logs)} 条会话 {session_id} 的历史日志。")
        ...
        logger.info(f"PTY 进程 (PID: {pty_process.pid}) 已为会话 {session_id} 启动，命令: '{pty_command_str}'，CWD: {pty_cwd or '默认'}")
        ...
        async with active_ptys_lock:
            if session_id in active_ptys:
                ...
                logger.error(f"会话 {session_id} 在存储 output_task 前消失。正在取消任务。")
                read_task.cancel()
        ...
        while True:
            if not (pty_process and pty_process.isalive()):
                ...
                if websocket.client_state == WebSocketState.CONNECTED:
                    await websocket.send_json({"type": "status", "message": "PTY 进程已终止。"})
                break 
                ...
                if msg_type == 'input':
                    ...
                    if isinstance(pty_input_data, str):
                        try:
                            ...
                            logger.error(f"向会话 {session_id} 的 PTY 写入时出错: {e_write}")
                            if not pty_process.isalive():
                                if websocket.client_state == WebSocketState.CONNECTED:
                                    await websocket.send_json({"type": "status", "message": "PTY 进程在写入错误后终止。"})
                                break
                    ...
                    try:
                        ...
                        logger.info(f"已将会话 {session_id} 的 PTY 大小调整为 {cols}x{rows}")
                    except Exception as e_resize:
                        logger.error(f"调整会话 {session_id} 的 PTY 大小时出错: {e_resize}")
                else:
                    logger.warning(f"收到来自会话 {session_id} 的未知消息类型 '{msg_type}'")

            except json.JSONDecodeError:
                logger.warning(f"收到来自会话 {session_id} 客户端的非 JSON 消息或格式错误的 JSON: {message_str}")
            except Exception as e_msg_proc:
                logger.error(f"处理会话 {session_id} 的消息时出错: {e_msg_proc}")
                if pty_process and not pty_process.isalive():
                    logger.warning(f"会话 {session_id} 的 PTY 进程已死亡。正在中断消息循环。")
                    if websocket.client_state == WebSocketState.CONNECTED:
                       await websocket.send_json({"type": "status", "message": "PTY 进程意外终止。"})
                    break
    
    except WebSocketDisconnect:
        logger.info(f"会话 {session_id} 的客户端已断开连接 (WebSocketDisconnect)。")
    except Exception as e_main:
        logger.error(f"handle_websocket_connection 中发生意外错误 (会话 {session_id}): {e_main}", exc_info=True)
    finally:
        ...
        if read_task and not read_task.done():
            ...
            try:
                ...
                logger.error(f"等待已取消的会话 {session_id} PTY 输出任务时出错: {e_task_cancel}")
        ...
        async with active_ptys_lock:
            if session_id in active_ptys:
                ...
                if pty_process_to_kill and pty_process_to_kill.isalive():
                    logger.info(f"正在终止会话 {session_id} 的 PTY 进程 (PID: {pty_process_to_kill.pid})。")
                    try:
        ...
        if websocket.client_state == WebSocketState.CONNECTED:
            try:
                await websocket.close()
                 ...
                 logger.warning(f"关闭会话 {session_id} 的 WebSocket 时发生运行时错误 (可能已自行关闭): {e_close}")
