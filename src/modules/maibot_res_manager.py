...
class MaiBotResourceManager:
    ...
    def create_emoji(self, instance_id: str, emoji_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        创建新的表情包记录
        ...
        try:
            ...
            with conn:
                ...
                if row:
                    ...
                    return {"status": "failed", "message": f"未找到ID为 {emoji_id} 的表情包"}
    ...
    def get_emoji_by_hash(self, instance_id: str, emoji_hash: str) -> Dict[str, Any]:
        ...
        try:
            ...
            with conn:
                cursor = conn.cursor()
                cursor.execute("SELECT * FROM Emoji WHERE emoji_hash = ?", (emoji_hash,))
                row = cursor.fetchone()
                ...
                if row:
                    ...
                    return {"status": "failed", "message": f"未找到哈希为 {emoji_hash} 的表情包"}
                    
        except Exception as e:
            logger.error(f"获取表情包信息时出错: {e}")
            return {"status": "failed", "message": f"获取失败: {str(e)}"}
    
    def search_emojis(self, instance_id: str, filters: Dict[str, Any] = None, 
    ...
    def update_emoji(self, instance_id: str, emoji_id: int, update_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        更新表情包信息
        ...
        try:
            ...
            with conn:
                ...
                allowed_fields = [
                    'full_path', 'format', 'emoji_hash', 'description', 'query_count',
                    'is_registered', 'is_banned', 'emotion', 'register_time',
                    'usage_count', 'last_used_time'
                ]
    ...
    def increment_emoji_query(self, instance_id: str, emoji_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            with conn:
                cursor = conn.cursor()
                cursor.execute("UPDATE Emoji SET query_count = query_count + 1 WHERE id = ?", (emoji_id,))
    ...
    def create_person_info(self, instance_id: str, person_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        创建新的用户信息记录
        ...
        try:
            ...
            with conn:
                ...
                # 检查 person_id 是否已存在
                cursor.execute("SELECT id FROM person_info WHERE person_id = ?", (person_data.get('person_id'),))
                if cursor.fetchone():
                    return {"status": "failed", "message": f"用户ID {person_data.get('person_id')} 已存在"}
                ...
                logger.info(f"成功创建用户信息记录，ID: {record_id}, 用户ID: {person_data.get('person_id')}")
                return {
                    "status": "success",
                    "message": "用户信息创建成功",
                    "record_id": record_id,
        ...
        try:
            ...
            with conn:
                ...
                if row:
                    ...
                    return {"status": "failed", "message": f"未找到ID为 {record_id} 的用户信息"}
                    
        except Exception as e:
            logger.error(f"获取用户信息时出错: {e}")
            return {"status": "failed", "message": f"获取失败: {str(e)}"}
    
    def get_person_info_by_person_id(self, instance_id: str, person_id: str) -> Dict[str, Any]:
        """
        根据用户唯一ID获取用户信息
        ...
        try:
            ...
            with conn:
                cursor = conn.cursor()
                cursor.execute("SELECT * FROM person_info WHERE person_id = ?", (person_id,))
                row = cursor.fetchone()
                ...
                if row:
                    ...
                    return {"status": "failed", "message": f"未找到用户ID为 {person_id} 的用户信息"}
                    
        except Exception as e:
            logger.error(f"获取用户信息时出错: {e}")
            return {"status": "failed", "message": f"获取失败: {str(e)}"}
    
    def get_person_info_by_platform_user(self, instance_id: str, platform: str, user_id: str) -> Dict[str, Any]:
        """
        根据平台和平台用户ID获取用户信息
        ...
        try:
            ...
            with conn:
                cursor = conn.cursor()
                cursor.execute("SELECT * FROM person_info WHERE platform = ? AND user_id = ?", (platform, user_id))
                row = cursor.fetchone()
                
                if row:
                    person_data = dict(row)
                    logger.info(f"成功获取用户信息，平台: {platform}, 用户ID: {user_id}")
                    return {"status": "success", "data": person_data}
                else:
                    return {"status": "failed", "message": f"未找到平台 {platform} 用户ID {user_id} 的用户信息"}
                    
        except Exception as e:
            logger.error(f"获取用户信息时出错: {e}")
            return {"status": "failed", "message": f"获取失败: {str(e)}"}
    
    def search_person_info(self, instance_id: str, filters: Dict[str, Any] = None, 
                          limit: int = 100, offset: int = 0) -> Dict[str, Any]:
        """
        搜索用户信息
                ...
                if filters:
                    ...
                    if 'has_person_name' in filters:
                        if filters['has_person_name']:
                            where_conditions.append("person_name IS NOT NULL AND person_name != ''")
                        else:
                            where_conditions.append("(person_name IS NULL OR person_name = '')")
    ...
    def update_person_info(self, instance_id: str, person_id: str, update_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        更新用户信息
        ...
        try:
            ...
            with conn:
                ...
                allowed_fields = [
                    'person_name', 'name_reason', 'platform', 'user_id', 'nickname',
                    'impression', 'short_impression', 'points', 'forgotten_points',
                    'info_list', 'know_times', 'know_since', 'last_know'
                ]
    ...
    def delete_person_info(self, instance_id: str, person_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            with conn:
                cursor = conn.cursor()
                cursor.execute("DELETE FROM person_info WHERE person_id = ?", (person_id,))
    ...
    def update_person_interaction(self, instance_id: str, person_id: str, 
                                 impression_update: str = None, 
                                 short_impression_update: str = None,
                                 points_update: str = None) -> Dict[str, Any]:
        """
    ...
    def get_emoji_count(self, instance_id: str, filters: Dict[str, Any] = None) -> Dict[str, Any]:
        """
        获取表情包记录总数
        ...
        try:
            ...
            with conn:
                ...
                logger.info(f"获取表情包总数成功，实例: {instance_id}, 总数: {total_count}")
                return {
                    "status": "success", 
    ...
    def get_emoji_batch(self, instance_id: str, batch_size: int = 50, offset: int = 0, 
        ...
        try:
            ...
            with conn:
                ...
                logger.info(f"批量获取表情包成功，实例: {instance_id}, 批次大小: {batch_size}, 偏移: {offset}, 返回数量: {len(emoji_list)}")
                return {
                    "status": "success",
                    "data": emoji_list,
    ...
    def get_person_info_count(self, instance_id: str, filters: Dict[str, Any] = None) -> Dict[str, Any]:
        """
        获取用户信息记录总数
        ...
        try:
            ...
            with conn:
                ...
                logger.info(f"获取用户信息总数成功，实例: {instance_id}, 总数: {total_count}")
                return {
                    "status": "success", 
                    "data": {"total_count": total_count},
    ...
    def get_person_info_batch(self, instance_id: str, batch_size: int = 50, offset: int = 0, 
                             filters: Dict[str, Any] = None) -> Dict[str, Any]:
        """
        批量获取用户信息数据
        ...
        try:
            ...
            with conn:
                ...
                logger.info(f"批量获取用户信息成功，实例: {instance_id}, 批次大小: {batch_size}, 偏移: {offset}, 返回数量: {len(person_info_list)}")
                return {
                    "status": "success",
